...
function KeyRow({ name, secret, placeholder, value, setValue }) {
  ...
  return <div className="flex flex-row w-full">
    ...
    <div className="flex flex-row flex-1 border border-black rounded-sm overflow-x-scroll">
      ...
      />
      <button onClick={handleStore} className="min-w-[75px] bg-black text-white font-bold p-1">
        {isStored ? 'Unset' : 'Set'}
      </button>
    </div>
  </div>
...
function ToggleRow({ name, options, value, setValue }) {
  return <div className="flex flex-row">
    <div className="flex min-w-[100px] items-center justify-center">{name}</div>
    <div className="flex flex-row border border-black rounded-sm overflow-x-scroll">
      {options.map((opt, i) => {
        const optLower = opt.toLowerCase()
        const bgCol = (optLower == value) ? 'bg-black' : 'bg-white'
        const textCol = (optLower == value) ? 'text-white' : 'text-black'
        return <button key={opt} onClick={() => setValue(optLower)} className={`min-w-[75px] ${bgCol} ${textCol} font-bold p-1 border-r last:border-r-0 border-black`}>
          {opt}
        </button>
      })}
    </div>
...
function Settings({ settings, setSettings }) {
  ...
  return <div className="w-[70%] flex flex-col gap-2 m-4 text-sm">
