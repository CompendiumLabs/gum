...
function computeStackLayout(direc, children) {
    ...
    const getBounds = sz => cumsum(sz).map((b, i) => [b, b + sz[i]])
...
class Stack extends Group {
    constructor({ children: children0, direc, expand = true, align = 'center', spacing = 0, aspect, ...attr } = {}) {
        ...
        // compute layout
        const { bounds, aspect: aspect_ideal } = computeStackLayout(direc, children)
        aspect ??= aspect_ideal

        // assign child rects
        zip(children, bounds).forEach(([c, b]) => {
            c.spec.rect = join_lims({ [direc]: b })
        })
