...
function rangeMap(lim, x, clim = lim_base) {
    const [ lo, hi ] = lim
    const [ clo, chi ] = clim
    const f = (x - clo) / (chi - clo)
    return lo + f * (hi - lo)
}
...
class Context {
    ...
    map({ rect, aspect = null, expand = false, align = 'center', coord = coord_base } = {}) {
        ...
        let [ cx0, cy0, rw0, rh0 ] = rect_radial(prect0)

        // embed rect to aspect
        const [ rw, rh ] = embed_aspect([ rw0, rh0 ], aspect, expand)

        // align rect (left, right, center, <float>)
        const afrac = align_frac(align)
        const [ cx, cy ] = [
            cx0 + (2 * afrac - 1) * (rw - rw0),
            cy0 + (2 * afrac - 1) * (rh - rh0)
        ]
