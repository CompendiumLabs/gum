...
class ArrowHead extends Element {
    ...
    props(ctx) {
        ...
        // get arc points
        const size = 0.5 * (rx + ry)
        const [ dx1, dy1 ] = mul(delx, size)
        const [ dx2, dy2 ] = mul(dely, size)
...
class ArrowPath extends Group {
    constructor({ children: children0, pos_beg, pos_end, direc_beg, direc_end, arrow, arrow_beg, arrow_end, arrow_size = 0.03, ...attr0 } = {}) {
        ...
        console.log(direc_beg, direc_end)
...
class Node extends Frame {
    constructor({ children: children0, pos, size = 0.1, rounded = 0.05, ...attr0 } = {}) {
        ...
        super({ children: text, padding: 0.1, rounded, rect, ...attr })
    }
