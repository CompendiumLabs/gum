...
function embed_aspect(size, aspect, expand = false) {
    if (aspect == null) return size
    const [ w, h ] = size
    const [ aw, ah ] = [ abs(w), abs(h) ]
...
function rotate_rect(size, aspect, rotate, tol=0.001) {
    if (aspect == null && rotate == 0) return size
    const [ w0, h0 ] = size
    ...
    // const [ rw, rh ] = [ h * DW, h * DH ]
    return [ w, h ]
}

function rotate_aspect(aspect, rotate) {
    if (aspect == null || rotate == 0) return aspect
    const theta = d2r * rotate
    const SIN = abs(sin(theta))
    const COS = abs(cos(theta))
    const DW = aspect * COS + SIN
...
class Context {
    ...
    mapRange(direc0, climit) {
        const direc = get_orient(direc0)
        const [ clo, chi ] = climit ?? lim_base
    ...
    map({ rect, aspect = null, expand = false, align = 'center', rotate = 0, coord = coord_base } = {}) {
        ...
        // rotate rect inside
        const [ w, h ] = rotate_rect([ w0, h0 ], aspect, rotate)
...
// spec keys
const spec_keys = [ 'rect', 'aspect', 'expand', 'rotate', 'coord' ]
...
class Svg extends Group {
    constructor({ children: children0, size = outer_base, prec = prec_base, bare = false, filters = null, debug = false, ...attr } = {}) {
        ...
        size = embed_aspect(size, this.spec.aspect)
...
class Frame extends Group {
    constructor({ children: children0, padding = 0, margin = 0, border = 1, aspect, adjust = true, flex = false, shape, rounded, stroke, fill, ...attr0 } = {}) {
...
class Points extends Group {
    constructor({ children: children0, points, rad = 0.01, shape: shape0, ...attr0 } = {}) {
        ...
        const children = points.map(pr => {
            ...
            const s = shape(point_attr)
            s.spec.rect = radial_rect(p, r)
            return s
        })
...
class Latex extends Element {
    constructor({ children: children0, pos = pos_base, rad = rad_base, ...attr } = {}) {
        const text = check_string(children0)
...
class TitleFrame extends Frame {
    constructor({ children: children0, title, title_size = 0.075, title_fill = 'white', title_offset = 0, title_rounded = 0.1, title_border = 1, adjust = false, padding = 0, margin = 0, border = 1, aspect, ...attr0 } = {}) {
        const child = check_singleton(children0)
        const [title_attr0, border_attr0, frame_attr0] = prefix_split(['title', 'border'], attr0)
        ...
        const frame_attr = { margin, border, ...border_attr, ...frame_attr0 }
        ...
        // make outer frame
        const frame = new Frame({ children: child, padding, border: 0 })
        const group = new Group({ children: [ frame, title_box ], aspect: frame.spec.aspect })
...
class SymPoints extends Group {
    constructor({ children: children0, fx, fy, fs, fr, rad: rad0 = 0.01, shape: shape0, xlim, ylim, tlim, xvals, yvals, tvals, N, ...attr } = {}) {
        const shape = ensure_function(shape0 ?? (() => new Dot()))
        const fsize = is_number(rad0) ? (() => rad0) : rad0
...
class Arrow extends Group {
    constructor({ children: children0, direc, pos = [0.5, 0.5], head = 0.3, tail = 2.0, shape = 'arrow', graph = true, ...attr0 } = {}) {
...
class Scale extends Group {
    constructor({ children: children0, direc, locs, lim = lim_base, coord, ...attr } = {}) {
        direc = get_orient(direc)
...
class Legend extends Place {
    constructor({ children: children0, lines, badgewidth = 0.1, vspacing = 0.1, hspacing = 0.025, rect, pos, rad, ...attr0 } = {}) {
...
class Plot extends Group {
    constructor({
        ...
        const icoord = [ xmin, ymax, xmax, ymin ]
        ...
        if (xaxis === true) {
            xaxis = new HAxis({ ticks: xticks, pos: ymin, lim: xlim, coord: icoord, tick_size: xtick_size, ...xaxis_attr })
        } else if (xaxis === false) {
            xaxis = null
        }
        ...
        if (yaxis === true) {
            yaxis = new VAxis({ ticks: yticks, pos: xmin, lim: ylim, coord: icoord, tick_size: ytick_size, ...yaxis_attr })
        } else if (yaxis === false) {
            yaxis = null
        }
        ...
        // fill background
        if (border != null) {
            border = new Rect({ rect: coord, stroke_width: border, ...border_attr })
        }
        ...
        const elems1 = [ border, xgrid, ygrid, ...elems, xaxis, yaxis ].filter(z => z != null)
