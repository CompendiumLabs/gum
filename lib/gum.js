...
class Element {
    constructor({ tag, unary, children, pos, rad, ...attr } = {}) {
        ...
        if (pos != null && rad != null) this.spec.rect ??= radial_rect(pos, rad)
...
class Group extends Element {
    constructor({ children: children0, coord, aspect, tag = 'g', clip = false, ...attr } = {}) {
...
class Frame extends Group {
    constructor({ children: children0, padding = 0, margin = 0, border = 0, aspect, adjust = true, flex = false, shape, rounded, stroke, fill, border_none, coord, ...attr0 } = {}) {
...
class ArrowPath extends Group {
    constructor({ children: children0, pos1, pos2, dir1, dir2, arrow, arrow_beg, arrow_end, arrow_size = 0.03, coord, ...attr0 } = {}) {
...
class Node extends Frame {
    constructor({ children: children0, label, rad = 0.1, padding = 0.1, border = 1, rounded = 0.05, aspect: aspect0, ...attr0 } = {}) {
        ...
        const text = is_element(children0) ? children0 : new MultiText({ children: children0, ...text_attr })
...
class Bars extends Group {
    constructor({ children: children0, sizes, direc = 'v', width = 0.9, zero = 0, color = 'lightgray', ...attr0 } = {}) {
        direc = ensure_orient(direc)
        const [ bar_attr, attr ] = prefix_split([ 'bar' ], attr0)

        // make individual bars
        const children = children0 ?? sizes.map((b, i) => {
            const [ size, fill ] = is_scalar(b) ? [ b, color ] : b
            return new RoundedRect({ fill, size, ...bar_attr })
        })
        ...
        children.forEach((child, i) => {
            const { size } = child.attr
            child.spec.rect = join_lims({
                [direc]: [ zero, size ],
                [idirec]: [ i - width2, i + width2 ],
            })
        })
...
class Graph extends Group {
    constructor({ children: children0, coord, aspect, padding = 0, flex = false, flip = true, ...attr } = {}) {
        ...
        const coord0 = outer_limits(elems, padding) ?? coord_base
...
class Plot extends Group {
    constructor({
        ...
        const [ xtick_size1, ytick_size1 ] = [ xtick_size * yrange, ytick_size * xrange ]
        ...
        if (xaxis === true) {
            ...
            const xaxis_rect = join_lims({ h: xlim, v: [ ymin - xtick_size1, ymin + xtick_size1 ] })
        ...
        if (yaxis === true) {
            const tick_pos = invert_axispos(axis_tick_pos)
            const label_pos = invert_axispos(axis_tick_label_pos)
            const yaxis_rect = join_lims({ h: [ xmin - ytick_size1, xmin + ytick_size1 ], v: ylim })
