...
class Frame extends Group {
    constructor({ children: children0, padding = 0, margin = 0, border = 1, aspect, adjust = true, flex = false, shape, rounded, stroke, fill, border_none, ...attr0 } = {}) {
        const child = check_singleton(children0)
        const [border_attr, attr] = prefix_split(['border'], attr0)

        // tailwind style rounded
        if (padding === true) padding = 0.1
        if (margin === true) margin = 0.1
        if (rounded === true) rounded = 0.05
        if (border_none === true) border = 0
...
class Plot extends Group {
    constructor({
        ...
        if (xaxis === true) {
            const label_pos = invert_axispos(tick_label_pos)
            const xaxis_rect = join_lims({ v: [ ymin - xtick_size, ymin + xtick_size ], h: xlim })
            xaxis = new HAxis({ ticks: xticks, lim: xlim, rect: xaxis_rect, label_pos, ...xaxis_attr })
            fg_elems.push(xaxis)
        } else if (xaxis === false) {
            xaxis = null
        }
        ...
        if (yaxis === true) {
            const label_pos = invert_axispos(tick_label_pos)
            const yaxis_rect = join_lims({ v: ylim, h: [ xmin - ytick_size, xmin + ytick_size ] })
            yaxis = new VAxis({ ticks: yticks, lim: ylim, rect: yaxis_rect, label_pos, ...yaxis_attr })
            fg_elems.push(yaxis)
        } else if (yaxis === false) {
            yaxis = null
        }
        ...
        if (border != null) {
            ...
            border = new Rect({ stroke_width: border, ...border_attr })
            fg_elems.push(border)
        }
