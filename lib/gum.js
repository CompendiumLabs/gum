...
class Place extends Group {
    constructor({ children: children0, pos, rad, rect, aspect, expand, align, rotate, invar, coord, ...attr } = {}) {
        const child = check_singleton(children0)
        child.spec.rect = rect ?? radial_rect(pos, rad)
        child.spec.aspect = aspect
        child.spec.expand = expand
        child.spec.align = align
        child.spec.rotate = rotate
        child.spec.invar = invar
        child.spec.coord = coord
...
class Arrowhead extends Polyline {
    constructor({ direc, angle = 60, base = false, ...attr } = {}) {
        // get size and angle offsets
        const [ anglex, angley ] = [ -direc - angle / 2, -direc + angle / 2 ]
        const [ dx1, dy1 ] = [ cos(d2r * anglex), sin(d2r * anglex) ]
        const [ dx2, dy2 ] = [ cos(d2r * angley), sin(d2r * angley) ]

        // generate arrowhead polygon
        const points = [
            [ 0.5 - 0.5 * dx1, 0.5 - 0.5 * dy1 ],
            [ 0.5, 0.5 ],
            [ 0.5 - 0.5 * dx2, 0.5 - 0.5 * dy2 ],
        ]

        // make base if requested
        if (base) points.push(points[0])
...
class Arrow extends Group {
    constructor({ children: children0, direc, tail = 0, shape = 'arrow', graph = true, ...attr0 } = {}) {
        ...
        if (is_scalar(direc)) {
            theta = direc
            const radians = d2r * theta
            direc = [ cos(radians), sin(radians) ]
        } else {
            theta = vector_angle(direc)
...
export {
    KEYS, VALS, Context, Element, Group, Svg, Defs, Style, Frame, Stack, VStack, HStack, Grid, Place, Flip, VFlip, HFlip, Anchor, Attach, Points, Absolute, Spacer, Ray, Line, UnitLine, HLine, VLine, Rect, RoundedRect, Square, Ellipse, Circle, Dot, Polyline, Polygon, Path, Command, MoveCmd, LineCmd, ArcCmd, CornerCmd, Arc, Triangle, Text, MultiText, Emoji, Latex, TextFrame, TitleFrame, Arrow, Field, SymField, Arrowhead, ArrowPath, Node, Edge, SymPath, SymFill, SymPoly, SymPoints, DataPath, DataPoints, DataFill, VMultiBar, HMultiBar, Bars, Scale, VScale, HScale, Labels, VLabels, HLabels, Axis, HAxis, VAxis, BoxLabel, Mesh, Graph, Plot, Legend, Note, Filter, Effect, DropShadow, Image, range, linspace, enumerate, repeat, meshgrid, lingrid, hexToRgba, palette, gzip, zip, reshape, split, concat, sum, prod, exp, log, sin, cos, min, max, abs, pow, sqrt, floor, ceil, round, atan, norm, clamp, mask, rescale, sigmoid, logit, smoothstep, rounder, random, uniform, normal, cumsum, pi, phi, r2d, d2r, none, blue, red, green, gray, is_string, is_array, is_object, is_function, is_element
}
