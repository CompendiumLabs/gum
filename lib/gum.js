...
class Bar extends RoundedRect {
    constructor({ size, fill = 'lightgray', ...attr } = {}) {
        super({ size, fill, ...attr })
    }
}
...
class Bars extends Group {
    constructor({ children: children0, sizes, direc = 'v', width = 0.9, zero = 0, color = 'lightgray', ...attr } = {}) {
        direc = ensure_orient(direc)

        // make individual bars
        const children = children0 ?? sizes.map(size => {
            return new Bar({ direc, size, fill: color, ...attr })
        })
...
class BarPlot extends Plot {
    constructor({ children, data, direc = 'v', aspect = 2, ygrid = true, ...attr0 } = {}) {
        const [ bar_attr, attr ] = prefix_split([ 'bar' ], attr0)

        // extract labels and sizes from children
        data = data ?? children.map(child => [ child.attr.label, child.attr.size ])
        const [ labs, sizes ] = zip(...data)

        // create bars
        const bars = new Bars({ children, direc, sizes, ...bar_attr })

        // determine axis ticks
        const tickdir = direc === 'v' ? 'x' : 'y'
        const [ tname, ticks ] = [ `${tickdir}ticks`, enumerate(labs) ]
        const [ lname, limit ] = [ `${tickdir}lim`, [ -0.75, data.length - 0.25 ] ]

        // pass on to Plot
        super({ children: bars, [tname]: ticks, [lname]: limit, aspect, ygrid, ...attr })
    }
}
