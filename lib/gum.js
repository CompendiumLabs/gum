...
class ArrowPath extends Group {
    constructor({ children: children0, pos1, pos2, dir1, dir2, arrow, arrow_beg, arrow_end, arrow_size = 0.03, coord, ...attr0 } = {}) {
        ...
        // pass to Group
        super({ children, clip: false, coord, ...attr })
    }
}

// this should provide a rect for positioning
class Node extends Frame {
    constructor({ children: children0, label, rad = 0.1, padding = 0.1, border = 1, rounded = 0.05, ...attr0 } = {}) {
        ...
        // pass to Frame
        super({ children: text, padding, border, rounded, rad, ...attr })
        this.label = label
    }
...
class Network extends Group {
    constructor({ children: children0, coord, ...attr } = {}) {
        ...
        const paths = edges.map(e => {
            ...
            const anchor1 = norm_direc(line1)
            const anchor2 = norm_direc(line2)
            const pos1 = node1.get_anchor(anchor1)
            const pos2 = node2.get_anchor(anchor2)
            const dir1 = anchor_direc(anchor1)
            const dir2 = mul(anchor_direc(anchor2), -1)
            return new ArrowPath({ pos1, pos2, dir1, dir2, coord, ...e.attr })
        })

        // pass to Graph
        super({ children: [...nodes, ...paths, ...other], clip: false, coord, ...attr })
    }
}
...
class Graph extends Group {
    constructor({ children: children0, coord, aspect, padding = 0, flex = false, flip = true, ...attr } = {}) {
        ...
        // split into placed and unplaced elements
        const unplaced = children.filter(e => e.spec.rect == null)
        const isplaced = children.filter(e => e.spec.rect != null)

        // map coordinate system to all elements
        const placed_group = new Group({ children: isplaced, clip: false, coord })
        unplaced.forEach(e => e.spec.coord = coord)

        // pass to Group
        super({ children: [ ...unplaced, placed_group ], aspect, ...attr })
    }
}
